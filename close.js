...
const ESTIMATED_OPEN_HOUR = "08.00/13.00";


const CLOSE_REASON = "Ada urusan penting yang harus diprioritaskan";
...
// Modal informasi (layanan tutup) - muncul setiap kali, tidak disimpan ke storage
...
// Modal informasi saat layanan BUKA (hanya 1x, disimpan ke localStorage)
// akan muncul lagi jika STORE_CLOSED berubah dari true ke false
function showStoreOpenInfo() {
    // Cek apakah modal open sudah pernah ditampilkan
...
// Reset storage ketika STORE_CLOSED berubah dari true ke false
function checkAndResetOpenModal() {
    const wasClosed = localStorage.getItem('was_store_closed');
    const currentClosed = STORE_CLOSED;
    
    // Jika sebelumnya tutup (true) dan sekarang buka (false), reset storage open modal
...
// Nonaktifkan fitur pembelian (BELI SAJA, KERANJANG TETAP BISA)
