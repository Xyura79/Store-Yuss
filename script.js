...
if (product.id === 20 || product.isAppBundle) {
    ...
    if (product.isApkOrderService || product.id === 17) {
...
// ================================================
// MODAL KONFIRMASI DEPOSIT (SETELAH BAYAR)
// ================================================
...
function showDepositConfirmationModal(orderId, nominal, username) {
    ...
    // Tombol cek status (manual update via API)
    ...
    processBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const message = `Halo kak, terima kasih sudah order di YussXy Store.\n\n` +
                `Berikut link APK Pembuat Logo yang sudah kakak beli:\n` +
...
// ================================================
// STATISTIK USER - TOTAL USER, TOP USER (SEMUA DEVICE), TOP PRODUK
// ================================================
...
async function loadUserStatistics() {
    ...
    try {
        ...
        if (allUsersData.success && allUsersData.users) {
            ...
            // TOTAL USER (semua device yang pernah transaksi atau punya username)
            ...
            // LOOP SEMUA USER (termasuk yang tidak punya username)
            for (const user of allUsers) {
                let totalSpent = 0;
                let displayName = user.username;
                
                // Jika tidak punya username, pakai device_id (dipotong 8 digit)
                ...
                if (user.trx_id && Array.isArray(user.trx_id)) {
                    for (const trx of user.trx_id) {
                        ...
                        const productName = trx.product_name || "Produk Lain";
            ...
            // Urutkan berdasarkan total pembelian terbanyak (TOP USER)
            ...
            // Cari top pembelian (produk paling banyak dibeli)
            let topProduct = null;
    ...
    try {
        ...
        if (allUsersData.success && allUsersData.users) {
            ...
            for (const user of allUsers) {
                ...
                if (user.trx_id && Array.isArray(user.trx_id)) {
                    for (const trx of user.trx_id) {
                        ...
                        const productName = trx.product_name || "Produk Lain";
                        if (!productSales[productName]) {
                            productSales[productName] = { count: 0, total: 0, firstTrx: trx };
                        }
                        productSales[productName].count++;
                        productSales[productName].total += amount;
                    }
            ...
            if (userSpending.length > 0) {
                ...
                // KLIK PADA CARD (BUKAN TEKS)
                // KLIK PADA CARD (BUKAN TEKS)
if (topUserCard) {
    topUserCard.style.cursor = 'pointer';
    topUserCard.onclick = () => {
        // Sembunyikan nomor HP, cukup 5 digit pertama
            ...
            if (topProduct) {
                ...
                if (topProductCard) {
                    ...
                    topProductCard.onclick = () => {
                        ...
                        const firstTrx = topProductFullData?.firstTrx;
                        const productId = firstTrx?.product_id || '-';
                        const productType = firstTrx?.type || '-';
                        ...
                        const content = `
                            <div style="display: flex; flex-direction: column; gap: 12px;">
                                ...
                                <div class="detail-item">
                                    <span class="detail-label"><i class="ri-qr-code-line"></i> Product ID</span>
                                    <span class="detail-value">${productId}</span>
                                </div>
                                <div class="detail-item">
                                    <span class="detail-label"><i class="ri-price-tag-line"></i> Tipe</span>
...
function showAutoSosmedModal() {
    ...
    const serviceSelect = document.getElementById('autoServiceSelect');
    const serviceDetail = document.getElementById('autoServiceDetail');
    ...
    if (!tiktokRegex.test(target) && !tiktokLinkRegex.test(target)) {
        showToast('❌ Masukkan username TikTok dengan @ (contoh: @yuss_xy) atau link profil!', true);
        return;
    }
} else if (selectedAutoService.id === 5) {
    const instagramRegex = /https?:\/\/(www\.)?instagram\.com\/(p|reel)\/[a-zA-Z0-9_-]+/;
    if (!instagramRegex.test(target)) {
...
function renderProductsWithStoreStatus() {
    ...
    productsGrid.innerHTML = allProducts.map(product => {
        const isDisabled = product._disabled === true;
        ...
        if (isDisabled) {
            productName = `<span style="opacity:0.6;">${product.name}</span>`;
        }
        ...
        return `
            <div class="product-card ${disabledClass}" data-product-id="${product.id}" ${clickHandler}>
                <div class="product-image-wrapper">
                    ${badgeHtml}
                    <img src="${product.image}" alt="${product.name}" class="product-image" onerror="this.src='https://placehold.co/300x300/333/white?text=Error'">
                </div>
